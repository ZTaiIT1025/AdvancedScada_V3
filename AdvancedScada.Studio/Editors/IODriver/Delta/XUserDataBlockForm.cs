...
{
    public partial class XUserDataBlockForm : Management.Editors.XDataBlockForm
    {
        int TagsCount = 1;
        ...
        public void AddressCreateTagDVP(DataBlock db, bool IsNew, int TagsCount = 1)
        {
            ...
            if (chkCreateTag.Checked)
                ...
                {
                    var tg = new Tag() { TagId = i + 1,
                        ChannelId = int.Parse(txtChannelId.Text),
                        DeviceId = int.Parse(txtDeviceId.Text),
                        DataBlockId = int.Parse(txtDataBlockId.Text),
                        TagName =
                        $"TAG{i + TagsCount:d5}", Address = $"{txtPrefix.Text}{txtStartAddress.Value + i}", DataType =
                        $"{cboxDataType.SelectedItem}", Description = $"{txtDesc.Text} {i + 1}"
                    };
                    db.Tags.Add(tg);
                }
        }
        private void XUserDataBlockForm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                txtPrefix.Properties.Items.Clear();
                txtPrefix.Properties.Items.AddRange(DVP);
                if (db == null)
                {
                    Text = "Add DataBlock";
