    ...
    public partial class XUserChannelForm : AdvancedScada.Management.Editors.XChannelForm
    {
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var ConnType = $"{TYPES.SelectedItem}";
                xtraTabControl1.SelectedTabPageIndex = TYPES.SelectedIndex;
                dxErrorProvider1.ClearErrors();
                switch (ConnType)
                {
                    case "SerialPort":
                        if ("Finish".Equals(btnNext.Text))
                        {
                            DISerialPort dis = new DISerialPort()
                            {
                                ...
                                ChannelTypes = "LSIS_V2",
                                CPU = $"{cboxModel.SelectedItem}",
                    ...
                    case "Ethernet":
                        if ("Finish".Equals(btnNext.Text))
                        {
                            ...
                            die = new DIEthernet()
                            {
                                ...
                                ChannelTypes = "LSIS_V2",
                                CPU = $"{cboxModel.SelectedItem}",
                                Rack = int.Parse(txtLsBaseNumber.Text),
                                Slot = int.Parse(txtLsSlotNumber.Text),
        ...
        private void XUserChannelForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ch != null)
                {
                    ...
                    this.TYPES.SelectedItem = $"{ch.ConnectionType}";
                    LS_MODES.SelectedItem = $"{ch.Mode}";
                    ...
                    switch (ch.ConnectionType)
                    {
                        ...
                        case "Ethernet":
                            ...
                            txtLsBaseNumber.Text = string.Format("{0}", ch.Rack);
                            txtLsSlotNumber.Text = string.Format("{0}", ch.Slot);
                            break;
