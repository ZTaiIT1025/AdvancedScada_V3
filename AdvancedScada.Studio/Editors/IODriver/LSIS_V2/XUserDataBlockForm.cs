...
namespace AdvancedScada.XLSIS_V2.Core.UserEditors
{
    public partial class XUserDataBlockForm : AdvancedScada.Management.Editors.XDataBlockForm
    {
        ...
        public void AddressCreateTagXGK(DataBlock db, bool IsNew, int TagsCount = 1)
        {
            ...
            if (chkCreateTag.Checked)
            {
                ...
                for (var i = 0; i < txtAddressLength.Value; i++)
                {
                    var tg = new Tag()
                    {
                        ...
                        Address = UpdateAddress(string.Format(LSNetBase.GetAddressFormat("{0}",
                                    (LSDomainTypes)Enum.Parse(typeof(LSDomainTypes), txtDomain.Text),
                                     (IODriverDataTypes)Enum.Parse(typeof(IODriverDataTypes), cboxDataType.SelectedItem.ToString())), txtStartAddress.Value + i, txtStartAddress.Value, i, txtLsBaseNumber.Value, txtLsSlotNumber.Value)),
        ...
        private void XUserDataBlockForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (db == null)
                {
                    ...
                    txtDomain.Properties.Items.AddRange(Enum.GetNames(typeof(LSDomainTypes)));
                    cboxDataType.Properties.Items.AddRange(Enum.GetNames(typeof(IODriverDataTypes)));
                }
                else
                {
                    Text = "Edit DataBlock";
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (db == null)
                    {
                        ...
                        if (chkCreateTag.Checked) AddressCreateTagXGK(dbNew, true, TagsCount);

                        var log = new LoggerPLC.LoggerPLC(1, "DataBlock",
                            $"{DateTime.Now.ToLongTimeString()} {DateTime.Now.ToLongDateString()}", "Add to DataBlock ");
                    ...
                    else
                    {
                        ...
                        db.PrefixName = txtDomain.Text;
                        db.Length = (ushort)txtAddressLength.Value;
                        db.Description = txtDesc.Text;
                        db.DataType = $"{cboxDataType.SelectedItem}";

                        if (chkCreateTag.Checked) AddressCreateTagXGK(db, false, TagsCount);

                        var log = new LoggerPLC.LoggerPLC(1, "DataBlock",
                            $"{DateTime.Now.ToLongTimeString()} {DateTime.Now.ToLongDateString()}", "Update to DataBlock ");
