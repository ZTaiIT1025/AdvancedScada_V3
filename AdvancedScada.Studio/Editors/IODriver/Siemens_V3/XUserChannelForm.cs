                ...
                switch (ConnType)
                {
                    case "SerialPort":
                        if ("Finish".Equals(btnNext.Text))
                        {
                            ...
                            if (ch == null)
                            {
                                //LoggerPLC log = new LoggerPLC(1, "Channel", string.Format("{0} {1}", DateTime.Now.ToLongTimeString(), DateTime.Now.ToLongDateString()), "Add to Channel ");
